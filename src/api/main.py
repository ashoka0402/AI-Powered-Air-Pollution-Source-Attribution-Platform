...
Runs with deterministic demo fixtures by default. Replace DemoRepository with
Person 1's PostGIS repository and Person 2's pipeline adapter for production.
"""
...
app = FastAPI(title="VAYU Air Intelligence API", version="1.0.0", description="Person 3 API; demo fixtures are synthetic.")
...
SOURCES = [
    {"source_type":"traffic","probability":0.36,"contribution":0.36},
    {"source_type":"construction","probability":0.17,"contribution":0.17},
    {"source_type":"industrial","probability":0.12,"contribution":0.12},
    {"source_type":"road_dust","probability":0.09,"contribution":0.09},
    {"source_type":"other","probability":0.26,"contribution":0.26},
]
EVENTS = [
    {"event_id":"EVT-DEL-001","city":"Delhi NCR","pollutant":"PM2.5","observed_value":92,"baseline_value":48,"severity":"high","status":"active","primary_source_type":"traffic","confidence":0.53},
    {"event_id":"EVT-AHM-002","city":"Ahmedabad","pollutant":"PM10","observed_value":105,"baseline_value":61,"severity":"moderate","status":"active","primary_source_type":"construction","confidence":0.48},
    {"event_id":"EVT-MUM-003","city":"Mumbai","pollutant":"PM2.5","observed_value":48,"baseline_value":29,"severity":"moderate","status":"active","primary_source_type":"traffic","confidence":0.51},
]
REPORTS: list[dict] = []
...
def health():
    return {"status":"ok","service":"vayu-api","data_mode":"synthetic_demo"}
...
def get_pollution_history(city: str = "Delhi NCR", pollutant: Literal["aqi","pm25","pm10","no2"] = "pm25", hours: int = Query(24, ge=1, le=168)):
    current = _city(city)[pollutant]
    now = datetime.now(timezone.utc)
    return {"data_mode":"synthetic_demo","city":city,"pollutant":pollutant,"points":[{"timestamp":(now-timedelta(hours=hours-i)).isoformat(),"value":round(max(0,current*(0.78+0.22*((i%7)/6))),1)} for i in range(hours+1)]}
...
def get_event_details(event_id: str):
    ...
    return {"data_mode":"synthetic_demo","event":event,"detected_at":datetime.now(timezone.utc).isoformat(),"evidence":["Concentration above demo baseline","Spatial candidate score available","Weather evidence not yet connected to live provider"]}
...
def get_neighbourhood_exposure(city: str):
    c = _city(city)
    aqi = c["aqi"]
    band = "high" if aqi > 150 else "moderate" if aqi > 100 else "low"
    # Explicitly illustrative until gridded population + validated concentration surfaces are connected.
...
def get_event_recommendations(event_id: str):
    ...
    if not event: raise HTTPException(404, "Event not found")
    actions = {
        "traffic":["Review congestion management near the affected area","Check public-transport and idling-reduction measures"],
        "construction":["Inspect active construction dust controls","Verify site covering and water-suppression practices"],
        "industrial":["Review nearby facility operating and emissions records","Arrange targeted inspection where legally authorized"],
        "road_dust":["Assess road-cleaning and dust-suppression needs","Inspect unpaved shoulders and high-dust segments"],
    }.get(event["primary_source_type"],["Review local source and meteorological evidence"])
...
def submit_citizen_report(payload: CitizenReportIn):
    report = {**payload.model_dump(),"report_id":f"CR-{uuid4().hex[:10].upper()}","timestamp":datetime.now(timezone.utc),"credibility_score":0.5,"status":"submitted"}
    REPORTS.append(report)
    return report

@app.get("/api/v1/citizen-reports")
def list_citizen_reports():
    return {"count":len(REPORTS),"reports":REPORTS,"data_mode":"in_memory_demo"}
